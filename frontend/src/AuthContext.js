  ...
  return (
    <AuthContext.Provider value={value}>
      {loading ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
          <div style={{ background: 'white', padding: '40px', borderRadius: '16px', maxWidth: '500px' }}>
            <h2 style={{ color: '#dc2626', marginBottom: '16px' }}>Connection Error</h2>
            <p style={{ color: '#64748b', marginBottom: '16px' }}>{error}</p>
            <p style={{ fontSize: '14px', color: '#94a3b8' }}>Please check your Supabase credentials in frontend/.env.local</p>
          </div>
        </div>
      ) : children}
